...
# DATA --------------------------------------------------------------------
  ...
  data <- reactive({
    ...
    df <- last_file(cnf) %>%
      ...
      .[!is.na(school)] %>%
      unique() %>%
      filter_comment(comment = users$comment)
  ...
  output$admin_upload_data_title <- renderUI({
    admin_upload_data_title()
  })
  output$admin_upload_data_select <- renderUI({
    admin_upload_data_select()
  })
  output$admin_upload_data_name <- renderUI({
    admin_upload_data_name()
  })
  output$admin_upload_data_process <- renderUI({
    admin_upload_data_process()
  })
  ...
  output$admin_files_title <- renderUI({
    admin_files_title()
  })
  output$admin_files_list <- renderUI({
    admin_files_list()
  })
  output$admin_files_delete <- renderUI({
    admin_files_delete()
  })
  output$admin_files_delete_check <- renderUI({
    admin_files_delete_check()
  })
  ...
  output$admin_dataset_title <- renderUI({
    admin_dataset_title()
  })
  output$admin_dataset_create <- renderUI({
    admin_dataset_create()
  })
  output$admin_dataset_list <- renderUI({
    admin_dataset_list()
  })

  output$admin_dataset_delete <- renderUI({
    admin_dataset_delete()
  })
  output$admin_dataset_update <- renderUI({
    admin_dataset_update()
  })

  output$admin_dataset_delete_check <- renderUI({
    admin_dataset_delete_check()
  })

  output$admin_dataset_content <- DT::renderDT({
    admin_dataset_content()
  })
  output$admin_dataset_list_title <- renderUI({
    admin_dataset_list_title()
  })
  output$admin_content_title <- renderUI({
    admin_content_title()
  })
  ...
  dataset <- reactive({
    ...
    if (as.logical(auth_out_list$admin)) {
      lslatr(cnf$preprocess$output_path) %>%
        .[rn == last_file(cnf)] %>%
        .[, c("rn", "mtime"), with = F] %>%
        .[, file := basename(rn) %>% map_chr(~ strsplit(., ".RDS") %>%
          unlist() %>%
          head(1))] %>%
        .[, c("file", "mtime"), with = F] %>%
  ...
  reactive_title_comp <- reactive({
    ...
    if (comp_external) {
      ...
      same_filter <- all(map2_lgl(
        .x = filter_comp, .y = filter(),
  ...
  output$title_comp <- renderUI({
    reactive_title_comp()
  })
  output$title_facet <- renderUI({
    reactive_title_facet()
  })
  ...
  output$subtitle_left <- renderUI({
    reactive_title_facet_left()
  })
  output$subtitle_right <- renderUI({
    reactive_title_facet_rigth()
  })
  ...
  selected_dataset <- reactive({
    auth_out_list <- reactiveValuesToList(auth_out)
    if (as.logical(auth_out_list$admin)) {
      downloadLink("download_data", "Descargar Dataset")
    } else {
      div()
    }
  })
  ...
  get_filename <- function(filename, cnf) {
    cnf$preprocess$output_path %>%
      lslatr() %>%
      .[basename(rn) == paste0(filename, ".RDS")] %>%
      .[, mtime %>% head(1)] %>%
      as.character() %>%
      gsub(" ", "T", .) %>%
      paste0(filename, "_", ., ".xlsx")
  }

  download_file <- function(filename, cnf) {
    file.path(cnf$preprocess$output_path, paste0(file, ".RDS")) %>%
      readRDS() %>%
      .[!is.na(school)] %>%
      unique() %>%
      .[, label := factor(y,
      ...
      .[, -c("timestamp", "y"), with = F] %>%
      unique() %>%
      setkey(school, course, group, uid) %>%
      .[] %>%
      fwrite(get_filename(filename, cnf))
  }



  output$download_data <- downloadHandler(
    filename = get_filename(input$admin_dataset_list, cnf),
    content = download_file(input$admin_dataset_list, cnf)
  )
}
